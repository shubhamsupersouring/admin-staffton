...
const OrganizationDetails = () => {
  ...
  const handleUpdateStatus = async (status, reason = null) => {
    ...
    try {
      if (status === 'suspended') {
        ...
        await organizationService.reactivateOrganization(id, { reason });
      ...
      toast.error(error.response?.data?.message || `Failed to ${status} organization`);
    } finally {
      setUpdating(false);
    }
  };
  ...
  const getStatusPill = (status) => {
    ...
    switch (s) {
      ...
      case 'suspended':
        return <span className={`${styles.statusPill} ${styles.rejected}`}>Suspended</span>;
      default:
        ...
        <div className={styles.headerActions}>
          ...
          {organisation.verification_status === 'approved' && (
            <button
              className={styles.rejectBtn}
              disabled={updating}
              onClick={() => setConfirmModal({ isOpen: true, type: 'suspended', selectedReason: '', customReason: '' })}
            >
              <AlertCircle size={18} /> Suspend
            </button>
          )}
          {organisation.verification_status === 'suspended' ? (
            <button
              className={styles.approveBtn}
              disabled={updating}
              onClick={() => setConfirmModal({ isOpen: true, type: 'reactivate', selectedReason: '', customReason: '' })}
            >
              <ShieldCheck size={18} /> Reactivate
            </button>
          ) : (
            <button
              className={styles.approveBtn}
      ...
      >
        <div className={styles.modalContent}>
          {confirmModal.type === 'approved' ? (
            ...
            <div className={styles.confirmWrapper}>
              <ShieldAlert size={40} className={styles.warnIcon} style={{ color: '#ef4444' }} />
              <p>Are you sure you want to suspend <strong>{organisation.name}</strong>? This will restrict their access to the platform.</p>
              <div className={styles.customReasonArea}>
                <label className={styles.modalLabel}>Reason for Suspension</label>
                <textarea
              ...
              <div className={styles.modalActions}>
                ...
                <button
                  ...
                  disabled={updating}
                  onClick={() => handleUpdateStatus('suspended', confirmModal.customReason)}
                >
                  {updating ? 'Processing...' : 'Confirm Suspend'}
                </button>
              </div>
            </div>
          ) : confirmModal.type === 'reactivate' ? (
            <div className={styles.confirmWrapper}>
              <ShieldCheck size={40} className={styles.warnIcon} style={{ color: 'var(--teal)' }} />
              <p>Are you sure you want to reactivate <strong>{organisation.name}</strong>? This will restore their active status.</p>
              <div className={styles.customReasonArea}>
              ...
              <div className={styles.modalActions}>
                ...
                <button
                  className={styles.modalConfirm}
                  disabled={updating}
                  onClick={() => handleUpdateStatus('reactivate', confirmModal.customReason)}
                >
                  {updating ? 'Processing...' : 'Confirm Reactivate'}
                </button>
              </div>
            </div>
          ) : (
            <div className={styles.rejectWrapper}>
              <p className={styles.modalSubTitle}>Please select a reason for {confirmModal.type === 'suspended' ? 'suspending' : 'rejecting'} this organization.</p>
              ...
              {confirmModal.selectedReason === 'Others' && (
                <div className={styles.customReasonArea}>
                  ...
                  <textarea
                    className={styles.reasonInput}
                    placeholder={`Provide detailed reason for ${confirmModal.type === 'suspended' ? 'suspension' : 'rejection'}...`}
                    value={confirmModal.customReason}
                    onChange={(e) => setConfirmModal({ ...confirmModal, customReason: e.target.value })}
                    autoFocus
                  />
                ...
                <button
                  ...
                  onClick={() => {
                    const finalReason = confirmModal.selectedReason === 'Others'
                      ...
                      : confirmModal.selectedReason;
                    handleUpdateStatus(confirmModal.type, finalReason);
                  }}
                >
                  {updating ? 'Processing...' : (confirmModal.type === 'suspended' ? 'Suspend Organization' : 'Reject Organization')}
                </button>
              </div>
            </div>
          )}
